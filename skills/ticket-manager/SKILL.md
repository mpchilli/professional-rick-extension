...
3.  **Create Child Tickets (ATOMIC IMPLEMENTATION)**:
    ...
    -   **CRITICAL (NO BUSYWORK)**: Every ticket MUST be an implementation task that results in a functional change or a testable unit of work.
...
4.  **Next Ticket Loop**:
    ...
    -   If found, set `current_ticket` and spawn a new Worker.
...
4.  **YIELD CONTROL**: You MUST output `[STOP_TURN]` and stop generating.
    ...
    -   **Failure to stop here results in recursive scope explosion.**
